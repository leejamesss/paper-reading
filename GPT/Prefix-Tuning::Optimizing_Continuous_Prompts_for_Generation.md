...
- **提示**：提示是指在任务输入前加上自然语言任务说明和一些例子，并从语言模型中生成输出。GPT-3 (Brown et al., 2020)使用人工设计的提示来适应不同的任务，这种框架被称为上下文学习（in-context learning）。Shin et al. (2020)提出了自动提示（AutoPrompt），通过搜索一系列离散的触发词并与每个输入连接起来，来从掩码语言模型中引出情感或事实知识。
